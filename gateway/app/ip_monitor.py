...
def get_real_client_ip(request: Request) -> str:
    ...
    优先级：
    1. X-Forwarded-For (通用CDN/代理) - 取第一个非内网IP
    2. X-Real-IP (Nginx)
...
class IpMonitor:
    ...
    def _refresh_blocked_cache(self):
        ...
        if now - self._last_blocked_refresh < 60:
            return
        try:
            rows = fetch_all("SELECT ip FROM ip_blocked WHERE unblocked_at IS NULL")
            self._blocked_cache = {row["ip"]: 0 for row in rows}
    ...
    def record_request(self, client_ip: str, client_id: str, key_prefix: str = "", user_agent: str = ""):
        ...
        # 更新内存缓存
        self._ip_client_map[client_ip].add(client_id)
        if key_prefix:
            self._ip_key_map[client_ip].add(key_prefix)
        self._ip_timestamps[client_ip].append(now_ts)
        self._ip_client_count[client_ip] = len(self._ip_client_map[client_ip])
        ...
        try:
            ...
            if existing:
                client_ids = set(json.loads(existing.get("client_ids", "[]")))
                client_ids.add(client_id)
                uas = set(json.loads(existing.get("user_agents", "[]")))
                if user_agent:
                    uas.add(user_agent)
                ...
                uas = {user_agent} if user_agent else set()
                execute(
                    ...
                    (client_ip, json.dumps(list(client_ids)), now_str, now_str,
                     anomaly_score, json.dumps(anomaly_reasons), json.dumps(list(uas))),
                )
        except Exception as e:
    ...
    def _detect_anomalies(self, ip: str) -> tuple:
        ...
        # === 算法1: 同IP多账号检测 ===
        current_clients = self._ip_client_map.get(ip, set())
    ...
    def block_ip(self, ip: str, reason: str = "异常行为封禁"):
        """封禁IP"""
        try:
            now_str = utcnow()
            execute(
            ...
            self._blocked_cache[ip] = 0
            logger.warning(f"IP已封禁: {ip} reason={reason}")
        except Exception as e:
