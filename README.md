...
控制台 13 个页面：仪表盘、上游密钥（含明文 reveal）、模型管理、模型测试、代理池、下游客户、桶监控、请求日志、算法引擎（17 算法实时状态）、系统监控、系统配置、错误码、商用检测。
...
代理表每行带勾选框，表头勾选框一键全选（半选态自动显示），操作栏另有「全选 / 清空勾选」两个按钮。勾选若干代理后可：

| 操作 | 行为 |
|------|------|
| 测试选中 | 浏览器端 4 路并发依次打 `POST /gw/admin/proxies/{id}/test`，每行「最近探测」列实时显示 `排队中 → 测试中 → 通 / 不通`，操作栏显示 `测试中 3/10 · 通过 2 · 失败 1`，随时可「中止」（未开始的行标为已中止，已发出的请求被 `AbortController` 取消） |
| 启用选中 / 停用选中 | 一次 `PUT /gw/admin/proxies/status`（一条 `UPDATE ... WHERE id = ANY(...)`），只写状态确实需要变的行——已是目标态的既不写库也不记审计，返回 `changed` 为真实变更数、`missing` 为已不存在的 ID 数 |

- 单次上限 200 个代理；单个启停仍走 `PUT /gw/admin/proxies/{id}`，两条路径并存
- 批量测试为**前端编排**而非后端批处理：单次探测默认 10s 超时，服务端串行 N 个会让请求挂到 10N 秒且无进度可看
- 勾选态跨表格重绘保持，代理被删除后自动从勾选集剔除；切换页面会自动中止正在跑的批量测试
...
| 批量测试 / 启停代理 | 控制台「代理池」页勾选若干代理（表头勾选框一键全选）→「测试选中」（4 路并发、逐行出结果、可中止）或「启用选中 / 停用选中」。批量启停只写状态确实需要变的行，返回真实变更数。等价 API：`PUT /gw/admin/proxies/status`（`{proxy_ids, status}`），单次上限 200 个 |
...
```bash
# 后端单元测试（360 个：安全体系 / 管理员密码 / 时间戳契约 / 代理池 / 统一异常 / 模型测试 / 上游密钥批量添加 / 代理池批量添加与批量启停 / 模型管理覆盖层与别名层）
python3 -m venv .venv
.venv/bin/pip install -r gateway/requirements.txt pytest pytest-asyncio
.venv/bin/python -m pytest tests/ -v

# 前端静态检查（69 项：引用完整性 / 无内联事件 / XSS 纪律 / API 契约锚点 / 路由注册 / 代理池动作 / 上游密钥单个·批量双路径 / 代理池单个·批量双路径 / 代理池勾选批量测试与启停 / 模型测试守卫 / 模型管理与别名守卫 / 时间本地化渲染 / 已下线端点守卫）
...
- **代理池勾选批量操作**：「代理池」页表格加勾选列（表头勾选框一键全选，半选态用 `indeterminate` 表达）与操作栏「全选 / 清空勾选 / 测试选中 / 启用选中 / 停用选中 / 中止」。**批量启停走新端点 `PUT /gw/admin/proxies/status`**（`{proxy_ids, status}`）：ID 去重保序、上限沿用 `BULK_MAX_LINES`(200)、先 `SELECT ... WHERE id = ANY(%s)` 算出 `missing` 并**跳过已是目标态的行**（既不写库也不记审计，`changed` 是真实变更数），随后一条 `UPDATE ... WHERE id = ANY(%s)` 与 `insert_audit_many()` 同在一个 `asyncio.to_thread` 里、写完 `proxy_pool.invalidate()`——形状刻意与 `PUT /models/visibility` 对齐，而不是让浏览器循环打 N 次 `PUT /proxies/{id}`。该静态路径**必须注册在 `PUT /proxies/{proxy_id}` 之前**，否则同一 HTTP 方法下 `"status"` 会被当成 `proxy_id` 吞掉（有 pytest 契约按源码下标断言先后）。**批量测试反而不加后端端点**：单次探测默认 10s 超时，服务端串行 N 个会让请求挂到 10N 秒且全程无进度，所以复用现成的 `POST /proxies/{id}/test`，浏览器端起 4 个 worker 轮取队列（复用「模型测试」页那套 `AbortController` + 队列编排），每行「最近探测」列实时走 `排队中 → 测试中 → 通/不通`，操作栏显示 `测试中 3/10 · 通过 2 · 失败 1`，「中止」既取消在途请求也把未开始的行标为已中止；切换页面（`proxyTableWrap` 消失）自动收手，不再继续打代理。勾选态存在模块级 `proxySel` 里跨表格重绘保持，代理被删除后自动剔除；全选是直接改现有 checkbox 而非重画表格，避免抹掉正在跑的行内测试状态。单个启停仍走 `PUT /proxies/{proxy_id}`，两条路径并存
- **测试**：后端 360 passed（代理 URL/客户端/选路 21 个 + 代理凭据加密与三路派生隔离用例；新增 `utc_from_ts` 格式/取值 3 个 + 写库路径不得回退到本地时区写入的守卫 2 个；新增模型测试 45 个：提示词归一 / `max_tokens` 收敛 / 回复与错误提取三态 / 路由与常量契约 / `extra="forbid"` / 不外泄凭据的源码守卫；新增管理员密码 16 个：明文校验（含非 ASCII 与含 `$` 口令）/ 不做 strip 与大小写归一 / 空配置拒绝一切登录 / `[FATAL]` 文案须点名变量名 / 登录端点必须走统一校验函数 / 两个登录口与 requirements 不得残留哈希方案；新增上游密钥批量添加 30 个：逐行解析（行号含空行注释、CRLF、含空格、长度上下边界含端点、批内查重指向首次出现行、跳过原因绝不回显密钥）/ 自动命名（空库从 01 起、按最大值续排、其他前缀不串号、跳过已占用名、`nv-99` → `nv-100`、空白前缀回落默认、超长前缀截断、正则元字符前缀按字面量处理）/ 源码契约（单个添加路径保留、批量端点注册与鉴权、响应只带掩码前缀、行数上限、解密查重、双缓存失效、一密钥一审计行）；新增代理池批量添加 52 个 + 批量启停契约 12 个：逐行解析（行号含空行注释、CRLF、首尾空白、协议与主机名大小写归一、四种协议白名单全覆盖、端口边界 1/65535 含端点、`:0` 与 `:65536` 与非数字端口、缺协议/缺地址/缺端口、带路径与参数与 fragment、裸尾斜杠容忍、IPv6 拒收、有密码无用户名、跳过原因绝不回显原始行）/ 凭据保真（密码含裸 `@`、`%40`、裸 `:`、用户名含 `%3A`、中文凭据，以及 `解析 → build_proxy_url → 再解析` 往返逐字一致）/ 批内查重（四元组指向首次出现行、同端点同账号异口令算重复、同端点异账号不算重复、异协议不算重复、尾斜杠形态与裸形态互判重）/ 源码契约（单个添加路径保留、批量端点注册与鉴权、响应不带密码、行数上限、查重不解密、加密在线程内、快照失效、一代理一审计行、复用命名生成器；批量启停端点注册且**排在 `/proxies/{proxy_id}` 之前**、状态白名单、去重与行数上限、单条 `= ANY(%s)` UPDATE、跳过已是目标态的行、每代理一条审计、失效代理池快照、单个启停路径保留）；新增模型管理 76 个：模型 ID 归一与非法字符拒收、覆盖层应用（剔除隐藏 / 追加排序后的手动项 / 上游收录后不重复追加 / 脏数据跳过）、`all_known_models` 含被隐藏项、管理行的来源与状态标记及按 ID 与备注搜索、`is_call_blocked` 在开关两态下的行为、源码契约（五端点注册与鉴权、删除仅限手动项、取消隐藏不留无信息行、写操作走线程+失效缓存+审计、建表与默认设置、路由注册、白名单确已移除、纠错集合基于全量集合））；新增模型别名层 69 个：`resolve_alias_pure` 三级匹配（精确 / 大小写 / 去分隔符标准化）与未命中原样返回、空值与脏数据（target 为空）不改写、`force_mapping` 透传、`apply_aliases`（替换 / `keep_original` 并存 / 同 target 多别名 OR 语义 / 别名按名排序在原位追加 / `owned_by` 取 provider 段 / 其余字段继承 / 不改原对象 / target 被隐藏则别名条目也不出现）、`build_alias_rows`（排序、`target_missing`、按别名·目标·备注搜索）、管理行携带别名与按别名搜索、源码契约（两端点注册与鉴权、写操作走线程+失效缓存+审计、四个错误码及其校验顺序、撞名与 target 存在性都对照 `all_known_models`、大小写唯一索引与 upsert 前的旧写法清理、别名解析在纠错之前、`refresh_verified_models` 只出现一次且只吃真名、流式回写落在 JSON 成功分支内、非流式回写在写日志之前、`/v1/embeddings` 与 `probe` 均解析、建表与 `lower(alias)` 唯一索引）；前端静态检查 69 项（代理池单个/批量双路径动作齐全、`/proxies/bulk` 调用锚点、批量结果面板被两处复用且仍无 `innerHTML`、批量动作用 textarea 收多行；代理池动作与出网徽标守卫；新增代理池勾选批量六个动作齐全、`/proxies/status` 调用锚点、行 checkbox 与 `data-pid` 经 `esc()` 输出、表头 `data-all` 与三态 `indeterminate`、批量测试的 `AbortController`+`signal.aborted` 双检与并发池与页面切走收手、探测文案走 `textContent`；新增 `fmtTime` 本地化渲染与详情页无裸时间戳守卫；新增模型测试页动作齐全、自测密钥不落盘、可中止、`innerHTML` 只写静态模板、`max_tokens` 默认值前后端一致守卫；新增上游密钥单个/批量双路径动作齐全、`/upstreams/bulk` 调用锚点、`formModal` 支持 textarea、批量结果面板无 `innerHTML`；新增模型管理页动作齐全、搜索为前端即时过滤、隐藏即禁用开关、模型 ID 经 `esc()` 输出且行动作只带 `data-idx`，以及后端白名单已下线、`get_model_list()` 叠覆盖层、纠错基于全量集合、`model_disabled` 拦截四项守卫；新增别名动作齐全（`model-alias`/`alias-edit`/`alias-delete`）、`/models/alias` 双端点调用锚点、别名表与「别名」列经 `esc()`·`badge()` 输出且行动作只带 `data-idx`、同一搜索框过滤两张表且模型搜索命中别名、布尔用 `select` 未改 `core.js`、`target_missing` 红标，以及后端别名层七项守卫：列表流水线含别名层、解析在纠错之前、全链路真名、`/v1/embeddings` 与 `probe` 解析、`lower(alias)` 唯一索引、默认语义） + 代理凭据加密与三路派生隔离用例；新增 `utc_from_ts` 格式/取值 3 个 + 写库路径不得回退到本地时区写入的守卫 2 个；新增模型测试 45 个：提示词归一 / `max_tokens` 收敛 / 回复与错误提取三态 / 路由与常量契约 / `extra="forbid"` / 不外泄凭据的源码守卫；新增管理员密码 16 个：明文校验（含非 ASCII 与含 `$` 口令）/ 不做 strip 与大小写归一 / 空配置拒绝一切登录 / `[FATAL]` 文案须点名变量名 / 登录端点必须走统一校验函数 / 两个登录口与 requirements 不得残留哈希方案；新增上游密钥批量添加 30 个：逐行解析（行号含空行注释、CRLF、含空格、长度上下边界含端点、批内查重指向首次出现行、跳过原因绝不回显密钥）/ 自动命名（空库从 01 起、按最大值续排、其他前缀不串号、跳过已占用名、`nv-99` → `nv-100`、空白前缀回落默认、超长前缀截断、正则元字符前缀按字面量处理）/ 源码契约（单个添加路径保留、批量端点注册与鉴权、响应只带掩码前缀、行数上限、解密查重、双缓存失效、一密钥一审计行）；新增代理池批量添加 52 个：逐行解析（行号含空行注释、CRLF、首尾空白、协议与主机名大小写归一、四种协议白名单全覆盖、端口边界 1/65535 含端点、`:0` 与 `:65536` 与非数字端口、缺协议/缺地址/缺端口、带路径与参数与 fragment、裸尾斜杠容忍、IPv6 拒收、有密码无用户名、跳过原因绝不回显原始行）/ 凭据保真（密码含裸 `@`、`%40`、裸 `:`、用户名含 `%3A`、中文凭据，以及 `解析 → build_proxy_url → 再解析` 往返逐字一致）/ 批内查重（四元组指向首次出现行、同端点同账号异口令算重复、同端点异账号不算重复、异协议不算重复、尾斜杠形态与裸形态互判重）/ 源码契约（单个添加路径保留、批量端点注册与鉴权、响应不带密码、行数上限、查重不解密、加密在线程内、快照失效、一代理一审计行、复用命名生成器）；新增模型管理 76 个：模型 ID 归一与非法字符拒收、覆盖层应用（剔除隐藏 / 追加排序后的手动项 / 上游收录后不重复追加 / 脏数据跳过）、`all_known_models` 含被隐藏项、管理行的来源与状态标记及按 ID 与备注搜索、`is_call_blocked` 在开关两态下的行为、源码契约（五端点注册与鉴权、删除仅限手动项、取消隐藏不留无信息行、写操作走线程+失效缓存+审计、建表与默认设置、路由注册、白名单确已移除、纠错集合基于全量集合））；新增模型别名层 69 个：`resolve_alias_pure` 三级匹配（精确 / 大小写 / 去分隔符标准化）与未命中原样返回、空值与脏数据（target 为空）不改写、`force_mapping` 透传、`apply_aliases`（替换 / `keep_original` 并存 / 同 target 多别名 OR 语义 / 别名按名排序在原位追加 / `owned_by` 取 provider 段 / 其余字段继承 / 不改原对象 / target 被隐藏则别名条目也不出现）、`build_alias_rows`（排序、`target_missing`、按别名·目标·备注搜索）、管理行携带别名与按别名搜索、源码契约（两端点注册与鉴权、写操作走线程+失效缓存+审计、四个错误码及其校验顺序、撞名与 target 存在性都对照 `all_known_models`、大小写唯一索引与 upsert 前的旧写法清理、别名解析在纠错之前、`refresh_verified_models` 只出现一次且只吃真名、流式回写落在 JSON 成功分支内、非流式回写在写日志之前、`/v1/embeddings` 与 `probe` 均解析、建表与 `lower(alias)` 唯一索引）；前端静态检查 59 项（代理池单个/批量双路径动作齐全、`/proxies/bulk` 调用锚点、批量结果面板被两处复用且仍无 `innerHTML`、批量动作用 textarea 收多行；代理池动作与出网徽标守卫；新增 `fmtTime` 本地化渲染与详情页无裸时间戳守卫；新增模型测试页动作齐全、自测密钥不落盘、可中止、`innerHTML` 只写静态模板、`max_tokens` 默认值前后端一致守卫；新增上游密钥单个/批量双路径动作齐全、`/upstreams/bulk` 调用锚点、`formModal` 支持 textarea、批量结果面板无 `innerHTML`；新增模型管理页动作齐全、搜索为前端即时过滤、隐藏即禁用开关、模型 ID 经 `esc()` 输出且行动作只带 `data-idx`，以及后端白名单已下线、`get_model_list()` 叠覆盖层、纠错基于全量集合、`model_disabled` 拦截四项守卫；新增别名动作齐全（`model-alias`/`alias-edit`/`alias-delete`）、`/models/alias` 双端点调用锚点、别名表与「别名」列经 `esc()`·`badge()` 输出且行动作只带 `data-idx`、同一搜索框过滤两张表且模型搜索命中别名、布尔用 `select` 未改 `core.js`、`target_missing` 红标，以及后端别名层七项守卫：列表流水线含别名层、解析在纠错之前、全链路真名、`/v1/embeddings` 与 `probe` 解析、`lower(alias)` 唯一索引、默认语义）
