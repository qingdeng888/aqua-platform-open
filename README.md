...
| **工程化** | 测试基线 203 个后端单测 + 37 项前端静态检查；新增依赖 `httpx[socks]`，移除 `bcrypt` |
...
```bash
# 后端单元测试（203 个：安全体系 / 管理员密码 / 时间戳契约 / 代理池 / 统一异常 / 模型测试 / 上游密钥批量添加 / 代理池批量添加）
python3 -m venv .venv
.venv/bin/pip install -r gateway/requirements.txt pytest pytest-asyncio
.venv/bin/python -m pytest tests/ -v

# 前端静态检查（37 项：引用完整性 / 无内联事件 / XSS 纪律 / API 契约锚点 / 路由注册 / 代理池动作 / 上游密钥单个·批量双路径 / 代理池单个·批量双路径 / 模型测试守卫 / 时间本地化渲染 / 已下线端点守卫）
